...
OUTPUT_PATH = Path("docs/Cognitive_Emotion_Workflow_Architecture_Models.docx")
...
def build_document() -> Document:
    ...
    title = doc.add_heading(
        "Cognitive Emotion Intelligence & Adaptive Lifestyle System", level=0
    )
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER

    subtitle = doc.add_paragraph("Workflow Architecture Models (Chapter 4 Methodology)")
    ...
    doc.add_paragraph()
    doc.add_paragraph(
        "This document contains the implementation-aligned architecture flowchart models "
        "for the proposed multi-modal emotion intelligence system."
    )

    doc.add_heading("1) End-to-End System Workflow", level=1)
    doc.add_paragraph("Flowchart model in Mermaid format:")
    add_code_block(
        doc,
        """
flowchart TD
...
flowchart LR
    ...
    doc.add_heading("3) Adaptive Recommendation Feedback Loop", level=1)
    doc.add_paragraph("Flowchart model in Mermaid format:")
    add_code_block(
        doc,
        """
flowchart TD
    ...
    doc.add_heading("4) Data Collection and Analysis Pipeline", level=1)
    doc.add_paragraph("Flowchart model in Mermaid format:")
    add_code_block(
        doc,
        """
flowchart TD
    ...
    doc.add_paragraph(
        "Source flowchart definitions are also available in: docs/workflow_architecture.md"
    )
    return doc
