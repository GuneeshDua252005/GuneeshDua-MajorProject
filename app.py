...
APP_SUBTITLE = (
    "Single-file CEI project with multi-modal fusion, MobileNetV2 training, "
    "Grad-CAM explainability, adaptive recommendation, and digital twin logging."
)
...
HF_TEXT_MODEL = os.getenv("HF_TEXT_MODEL", "HuggingFaceH4/zephyr-7b-beta").strip() or "HuggingFaceH4/zephyr-7b-beta"
...
MOOD_QUESTION_BANK = {
    "happy": [
        "What part of your day created this positive feeling?",
        "How can you repeat one small action that supported this mood?",
        "Who could you share this progress or gratitude with today?",
    ],
    "sad": [
        "What feels heaviest for you right now?",
        "When did you last feel even slightly better, and what helped then?",
        "What is one very small step that would make the next hour easier?",
    ],
    "calm": [
        "What is currently helping you stay balanced?",
        "Which routine is protecting your focus or peace today?",
        "What would you like to maintain before stress builds again?",
    ],
    "energetic": [
        "Is this energy helping you move forward or making you feel overloaded?",
        "What is the best direction to channel this intensity right now?",
        "What boundary or pause would keep this energy productive?",
    ],
}
...
def get_chat_provider_name() -> str:
    if HF_TOKEN:
        return f"Hugging Face ({HF_TEXT_MODEL})"
    if openai_client is not None:
        return f"OpenAI ({OPENAI_CHAT_MODEL})"
    return "Local offline emotional coach"
...
def get_user_twin_summary(user: str, limit: int = 12) -> str:
    ...
    try:
        ...
        return "Digital Emotional Twin history could not be loaded."
    user_df = df[df["User"].astype(str) == str(user)].tail(limit)
    if user_df.empty:
        return "No Digital Emotional Twin history is available yet."
    fused_series = user_df.get("FusedMood", pd.Series(dtype=str)).fillna("calm").astype(str)
    dominant_mood = fused_series.value_counts().index[0] if not fused_series.empty else "calm"
    recent_moods = ", ".join(fused_series.tail(5).tolist()) or "none"
    last_title_series = user_df.get("RecommendedTitle", pd.Series(dtype=str)).dropna().astype(str)
    last_title = last_title_series.iloc[-1] if not last_title_series.empty else "no recommendation yet"
    feedback_series = user_df.get("Feedback", pd.Series(dtype=str)).fillna("").astype(str)
    latest_feedback = feedback_series.iloc[-1] if not feedback_series.empty and feedback_series.iloc[-1] else "not recorded"
    return (
        f"Dominant recent mood: {dominant_mood}. "
        f"Recent mood sequence: {recent_moods}. "
        f"Last recommendation: {last_title}. "
        f"Latest feedback: {latest_feedback}."
    )


def build_chat_system_prompt(current_mood: str, twin_summary: str) -> str:
    normalized_mood = current_mood if current_mood in MOOD_CHOICES else "calm"
    return textwrap.dedent(
        f"""
        You are an empathetic emotional AI guide inside a final-year major project.
        The user current mood is estimated as: {normalized_mood}.
        Digital twin summary: {twin_summary}

        Your job:
        1. Respond in a warm, human-sounding, practical tone.
        2. Ask three important reflective questions that match the user's mood.
        3. Suggest one small action for the next 10-20 minutes.
        4. Avoid medical diagnosis, legal advice, or crisis instructions.
        5. Keep the reply concise, supportive, and easy to act on.
        """
    ).strip()


def huggingface_chat(prompt: str, system: str) -> str | None:
    if not HF_TOKEN:
        return None
    payload = {
        "inputs": (
...
def openai_chat(prompt: str, system: str = "You are a helpful emotional AI assistant.") -> str | None:
    if not prompt.strip() or openai_client is None:
        return None
    try:
...
def local_emotional_coach(prompt: str, current_mood: str, twin_summary: str) -> str:
    normalized_mood = current_mood if current_mood in MOOD_CHOICES else "calm"
    questions = MOOD_QUESTION_BANK.get(normalized_mood, MOOD_QUESTION_BANK["calm"])
    return textwrap.dedent(
        f"""
        I am running in the free local support mode, so I will guide you without any paid API.

        Current mood focus: {normalized_mood.title()}
        Digital twin summary: {twin_summary}
        ...
        Small next step:
        - {FALLBACK_ACTIONS.get(normalized_mood, FALLBACK_ACTIONS['calm'])}

        You said:
        "{prompt.strip()}"

        Reply with your answers and I can continue the conversation from your mood perspective.
        """
    ).strip()


def emotional_ai_chat(user: str, prompt: str, current_mood: str = "calm") -> str:
    if not prompt.strip():
        return "Type a message first."
    normalized_mood = current_mood if current_mood in MOOD_CHOICES else simple_text_to_mood(prompt)
    twin_summary = get_user_twin_summary(user)
    system = build_chat_system_prompt(normalized_mood, twin_summary)
    user_prompt = textwrap.dedent(
        f"""
        User mood: {normalized_mood}
        Twin summary: {twin_summary}
        User message: {prompt}

        Respond with:
        - a short supportive reflection,
        - three important questions,
        - one next practical action.
        """
    ).strip()
    hf_response = huggingface_chat(user_prompt, system=system)
    if hf_response:
        return hf_response
    openai_response = openai_chat(user_prompt, system=system)
    if openai_response:
        return openai_response
    return local_emotional_coach(prompt, normalized_mood, twin_summary)
...
def main() -> None:
    ...
    st.markdown(
        "A single-file Streamlit project for multi-modal emotion detection, transfer learning, "
        "Grad-CAM explainability, adaptive recommendation, Digital Emotional Twin logging, "
        "free-or-local emotional chat, and a 100+ item structured resource catalog."
    )
    ...
    with st.sidebar:
        ...
        live_mic = st.checkbox("Enable short microphone capture (may need microphone support on Windows)")
    ...
    with analysis_tab:
        ...
        if result:
            st.info(f"Current mood context for chat: {result['fused_mood'].title()}")
        chat_input = st.text_input("Say something to the assistant", value="")
        if st.button("Send Chat"):
            active_mood = result["fused_mood"] if result else simple_text_to_mood(chat_input)
            st.write(emotional_ai_chat(user=user, prompt=chat_input, current_mood=active_mood))
    ...
    with setup_tab:
        ...
        st.markdown(
            textwrap.dedent(
                ...
                3. **Free API configuration**
                   - Preferred free chat provider: create a free Hugging Face account, create a read token,
                     and set `HF_TOKEN`.
                   - Optional Spotify search integration uses free developer credentials:
                     `SPOTIPY_CLIENT_ID` and `SPOTIPY_CLIENT_SECRET`.
                   - OpenAI is optional and not required for project execution.
        ...
        st.markdown("**Why Hugging Face is used for the free chat path**")
        st.markdown(
            textwrap.dedent(
                f"""
                - It provides free user tokens, so the emotional chat can run without a paid OpenAI account.
                - It supports open-weight text-generation models and works well for mood-aware question prompting.
                - If the Hugging Face endpoint is busy or not configured, this app automatically falls back to a local rule-based emotional coach.
        ...
        st.markdown(
            textwrap.dedent(
                ...
                If you later need a real APK, use a WebView wrapper, Kivy/Buildozer, or Chaquopy in a Linux build environment.
                """
            )
        )
