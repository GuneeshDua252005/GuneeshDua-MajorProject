...
APP_SUBTITLE = (
    ...
    "voice/text/face fusion, Digital Emotional Twin memory, and free Hugging Face support."
)
...
ENABLE_LOCAL_TRANSFORMERS = os.getenv("ENABLE_LOCAL_TRANSFORMERS", "0").strip().lower() in {"1", "true", "yes"}
...
FALLBACK_ACTIONS = {
    "happy": "Write one win, one gratitude, and one small goal so this state becomes repeatable.",
    "sad": "Pause for two minutes, breathe slowly, drink water, and message one trusted person.",
    "calm": "Protect your balance with a low-noise task and a short focus sprint.",
    "energetic": "Channel the intensity into one meaningful task or a short movement break.",
}
...
MOOD_QUESTION_BANK = {
    ...
    "sad": [
        ...
        "What helped even slightly the last time you felt like this?",
...
TONE_GUIDES = {
    "Therapist": "Warm, grounded, reflective, structured, and careful not to diagnose.",
    "Friendly": "Caring, conversational, realistic, and personally supportive.",
    "Motivational": "Energising, disciplined, forward-looking, and action focused.",
}
...
SEARCH_QUERIES = {
    "YouTube": {
        "happy": ["feel good playlist", "gratitude meditation", "upbeat study break songs", "confidence booster music"],
        "sad": ["comfort songs", "self compassion meditation", "gentle piano mood", "mindfulness for low mood"],
        "calm": ["lofi focus session", "rain sounds for studying", "box breathing guide", "desk meditation"],
        "energetic": ["power workout mix", "motivation speech short", "focus sprint music", "quick movement break"],
    },
    "Spotify": {
        "happy": ["happy playlist", "feel good hits", "good vibes only", "positive energy songs"],
        "sad": ["comfort songs", "gentle piano", "self care songs", "healing ambient music"],
        "calm": ["lofi beats", "peaceful piano", "deep focus", "meditation music"],
        "energetic": ["workout hits", "high energy mix", "motivation songs", "gym boost"],
    },
    "YouTube Music": {
        "happy": ["cheerful bollywood playlist", "happy indie songs", "weekend road trip music", "bright cafe playlist"],
        "sad": ["soft ghazal comfort songs", "slow rainy evening music", "quiet reflection songs", "reset after bad day mix"],
        "calm": ["brown noise focus", "deep work lofi", "peaceful coding soundtrack", "evening tea playlist"],
        "energetic": ["high bpm study music", "sports warmup songs", "power walk music", "energetic morning mix"],
    },
}

DATASET_GUIDE = [
    {"dataset": "FER2025", "year": "2025", "best_use": "Main facial training dataset", "note": "Use small samples on 8 GB RAM."},
    {"dataset": "EmoNet-Face-Big", "year": "2025", "best_use": "Experiments or pretraining-style sampling", "note": "Limit classes for demo speed."},
    {"dataset": "MER2024", "year": "2024", "best_use": "Report and future scope", "note": "Often requires manual access approval."},
    {"dataset": "MER2023", "year": "2023", "best_use": "Literature comparison", "note": "Use only if access is approved."},
]
...
def ensure_runtime_files() -> None:
    ...
    ensure_csv_file(RECOMMENDER_STATS_PATH, ["ItemId", "Exposures", "Likes", "Skips", "LastShown", "LastFeedback"])
...
def build_resource_catalog() -> tuple[CatalogEntry, ...]:
    ...
    for mood in MOOD_CHOICES:
        ...
        for source, mood_queries in SEARCH_QUERIES.items():
            for index, query in enumerate(mood_queries[mood], start=1):
                if source == "YouTube":
                ...
                entries.append(
                    CatalogEntry(
                        ...
                        mood=mood,
                        title=f"{mood.title()} {source} search: {query.title()}",
                        url=url,
                        source=source,
                        resource_type=f"{slugify(source)}_search",
                        playable=False,
                        tags=f"{mood},{slugify(source)},{slugify(query)}",
                        offline_fallback=fallback,
                    )
                )
...
def recommend_resource(user: str, mood: str, last_n: int = 5) -> dict[str, Any]:
    ...
    for item in candidates:
        ...
        scored.append((score + random.uniform(0.0, 0.05), item))
...
def analyze_text_emotion(text: str, allow_transformers: bool = False) -> dict[str, Any]:
    text = (text or "").strip()
    ...
    if allow_transformers:
        sentiment_pipeline = get_transformers_pipeline("sentiment-analysis", TEXT_SENTIMENT_MODEL)
        ...
        emotion_pipeline = get_transformers_pipeline("text-classification", TEXT_EMOTION_MODEL)
...
def build_dataloaders(dataset_root: Path, batch_size: int = 8):
    ...
    eval_transform = transforms.Compose([transforms.Resize((224, 224)), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
    ...
    if val_ds is not None and list(val_ds.classes) != class_names:
        raise RuntimeError("Validation classes must match the training classes exactly.")
    if test_ds is not None and list(test_ds.classes) != class_names:
        raise RuntimeError("Test classes must match the training classes exactly.")
...
def evaluate_loader(model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device) -> tuple[float, float]:
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_samples = 0
...
def classification_report_np(matrix: np.ndarray, class_names: list[str]) -> dict[str, Any]:
    ...
    for index, class_name in enumerate(class_names):
        ...
        report[class_name] = {"precision": round(precision, 4), "recall": round(recall, 4), "f1-score": round(f1, 4), "support": support}
...
def evaluate_model_on_loader(model: nn.Module, loader: DataLoader | None, class_names: list[str], device: torch.device) -> dict[str, Any]:
    if loader is None:
    ...
    return {"report": report, "confusion_matrix": matrix.tolist(), "roc_auc_macro": round(roc_auc_macro, 4) if roc_auc_macro is not None else None}
...
def train_emotion_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if progress_callback is not None:
            progress_callback(epoch / epochs, history[-1])
...
def emotion_fusion(face_result: dict[str, Any], voice_result: dict[str, Any], text_result: dict[str, Any], emoji_mood: str) -> tuple[str, dict[str, float], float]:
    ...
    scores[face_result.get("mood", "calm")] += weights["face"] * max(float(face_result.get("confidence") or 0.45), 0.30)
    scores[voice_result.get("mood", "calm")] += weights["voice"] * max(float(voice_result.get("confidence") or 0.45), 0.30)
    scores[text_result.get("mood", "calm")] += weights["text"] * max(float(text_result.get("confidence") or 0.45), 0.30)
...
def ethical_monitor_report(face_result: dict[str, Any], voice_result: dict[str, Any], text_result: dict[str, Any], fusion_scores: dict[str, float]) -> dict[str, Any]:
    notes: list[str] = []
    modalities_used = 0
    ...
    if face_result.get("method") == "opencv_heuristic":
        notes.append("Facial analysis is using an OpenCV heuristic until a .pth model is trained.")
    if modalities_used < 2:
        notes.append("Fusion confidence improves when at least two modalities are present.")
    notes.append("This system provides supportive guidance, not a clinical diagnosis.")
    ...
    return {"modalities_used": modalities_used, "confidence_band": confidence_band, "notes": notes}


def get_chat_provider_name() -> str:
    if HF_TOKEN:
        return f"Hugging Face Inference API ({HF_CHAT_MODEL})"
    if ENABLE_LOCAL_TRANSFORMERS:
        return f"Optional local transformers ({LOCAL_CHAT_MODEL})"
...
def build_chat_system_prompt(username: str, mood: str, tone_mode: str, twin_summary: str, intent: str, analysis_snapshot: str) -> str:
    ...
    return textwrap.dedent(
        f"""
        You are an emotionally intelligent therapeutic mascot guide.
        User name: {username}
        ...
        2. Ask exactly three meaningful questions.
        3. Suggest one practical action for the next 10 to 20 minutes.
        4. Avoid diagnosis and avoid generic FAQ wording.
        """
    ).strip()
...
def get_local_chat_pipeline():
    if not ENABLE_LOCAL_TRANSFORMERS:
        return None
...
def local_rule_based_response(question: str, mood: str, tone_mode: str, twin_summary: str, intent: str) -> str:
    ...
    return textwrap.dedent(
        ...
        Digital twin context: {twin_summary}
...
def generate_chat_response(username: str, question: str, current_mood: str, tone_mode: str, analysis: dict[str, Any] | None) -> tuple[str, str]:
    text_result = analyze_text_emotion(question, allow_transformers=ENABLE_LOCAL_TRANSFORMERS)
    ...
    if response:
        return response, get_chat_provider_name()
    return local_rule_based_response(question, active_mood, tone_mode, twin_summary, text_result["intent"]), get_chat_provider_name()
...
def streamlit_width_arg() -> dict[str, str]:
    try:
        version = getattr(st, "__version__", "0.0.0")
        major, minor, *_ = [int(part) for part in re.findall(r"\d+", version)[:2]]
        if (major, minor) >= (1, 50):
...
def render_mascot(mood: str, username: str, tone_mode: str, subtitle: str) -> None:
    ...
    st.markdown(
        f"""
        <style>
        .cei-card {{ background: {palette["bg"]}; border: 1px solid rgba(15,23,42,.08); border-radius: 22px; padding: 18px; box-shadow: 0 12px 24px rgba(15,23,42,.08); }}
        ...
        <div class="cei-card">
            ...
            <div class="cei-stage"><div class="cei-glow"></div><div class="cei-avatar">
                <div class="cei-eye cei-eye-left {eye_class}"></div><div class="cei-eye cei-eye-right {eye_class}"></div>{mouth_html}
            </div></div>
...
def live_analysis(user: str, emoji: str, image_bytes: bytes | None, audio_bytes: bytes | None, text_input: str, tone_mode: str, use_hf_text_models: bool):
    face_result = predict_face_emotion(image_bytes)
    voice_result = analyze_voice_emotion(audio_bytes, username=user)
    text_result = analyze_text_emotion(text_input, allow_transformers=use_hf_text_models)
    ...
    log_interaction(user, face_result["label"], face_result["mood"], voice_result["mood"], text_result["mood"], emoji_mood, fused_mood, state_score, tone_mode, recommendation)
    return result
...
def render_setup_tab() -> None:
    ...
    st.markdown(
        textwrap.dedent(
            ...
            2. Put only these required source files in it:
               - `app.py`
               - `requirements.txt`
               - `.gitignore`
            ...
            Python 3.11 is recommended for the smoothest Streamlit and PyTorch Windows experience. If you use Python 3.14
            before all ML wheels are available, pip may fail to install some packages from wheels.
            """
        )
    )
    st.markdown("### requirements.txt")
    st.code(
        "\n".join(["streamlit", "numpy", "torch", "torchvision", "opencv-python", "nltk", "SpeechRecognition", "pyttsx3", "transformers"]),
        language="text",
    )
    st.markdown("### Optional Hugging Face free token")
    st.markdown(
        textwrap.dedent(
            """
            - Create a free Hugging Face account.
            - Open Settings, then Access Tokens.
            ...
            The app still works without a token by using local rule-based emotional coaching.
            """
        )
    )
    st.markdown("### Dataset folder for PyTorch training")
    st.code(
        textwrap.dedent(
            """
            dataset/
    ...
    st.markdown(
        "Open the Streamlit URL on Android Chrome and use Add to Home Screen. For a real APK, wrap the same URL in a free Android Studio WebView project."
    )
    st.markdown("### Viva and future scope points")
    st.markdown(
        "- PyTorch EfficientNetV2-S with global average pooling and Grad-CAM\n"
        "- Multimodal fusion from face, voice, text, and emoji\n"
        "- Digital Emotional Twin logging\n"
        "- No-repeat recommender with feedback memory\n"
        "- Ethical AI confidence notes and non-diagnostic guidance\n"
        "- Future scope: lip sync, multilingual support, stronger speech emotion models, mobile packaging"
    )
    st.table(DATASET_GUIDE)


def run_cli() -> int:
    parser = argparse.ArgumentParser(description="PyTorch CEI major project utility")
    parser.add_argument("--export-catalog", action="store_true", help="Export the structured resource catalog CSV.")
    parser.add_argument("--catalog-path", default=str(CATALOG_EXPORT_PATH), help="Custom destination for the exported catalog CSV.")
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        use_hf_text_models = st.checkbox("Use local Hugging Face text classifiers", value=False)
        speak_reply = st.checkbox("Speak chatbot reply aloud locally", value=False)
        st.caption(f"Chat provider: {get_chat_provider_name()}")
        st.caption("For Windows 8 GB RAM: batch size 4-8 and demo epochs 1-3.")
    ...
    with tab_live:
        ...
        with left:
            ...
            if hasattr(st, "audio_input"):
                ...
                st.info("Your Streamlit version does not support browser microphone capture. Use typed text fallback.")
            text_input = st.text_area("Context message", height=140, placeholder="Type how you feel, what happened, or what you need help with.")
            if st.button("Analyze emotional state", type="primary"):
                with st.spinner("Running PyTorch and NLP multimodal analysis..."):
                    st.session_state["analysis_result"] = live_analysis(username, emoji, image_bytes, audio_bytes, text_input, tone_mode, use_hf_text_models)
                    st.session_state["feedback_saved"] = None
        with right:
        ...
        if result:
            ...
            with col_a:
                ...
                if result["face_result"].get("overlay_rgb") is not None:
                    st.image(result["face_result"]["overlay_rgb"], caption=f"Method: {result['face_result']['method']}", **streamlit_width_arg())
                else:
            ...
            with col_b:
                ...
                for note in result["ethical_report"]["notes"]:
                    st.caption(f"- {note}")
            ...
            st.write(
                f"Emotion: {text_result['emotion_label']} ({text_result['emotion_score']:.2f}) | "
                f"Sentiment: {text_result['sentiment_label']} ({text_result['sentiment_score']:.2f}) | Intent: {text_result['intent']}"
            )
    ...
    with tab_chat:
        ...
        render_mascot(active_mood, username, tone_mode, f"{username}, ask me anything. I will answer according to your emotional state and mindset.")
        ...
        if prompt:
            with st.chat_message("user"):
                st.write(prompt)
            reply, provider_name = generate_chat_response(username, prompt, active_mood, tone_mode, analysis)
            ...
            log_interaction(
                ...
                analysis["text_result"]["mood"] if analysis else simple_text_mood(prompt),
    ...
    with tab_train:
        ...
        st.caption("Expected local folders: dataset/train, dataset/val, and dataset/test. The trained model is saved as models/emotion_efficientnet_v2_s.pth.")
        ...
        if st.button("Train EfficientNetV2-S model"):
            ...
            try:
                with st.spinner("Training PyTorch EfficientNetV2-S on the local dataset..."):
                    _, history, evaluation, class_names = train_emotion_model(DATASET_DIR, batch_size, epochs, float(learning_rate), freeze_backbone, update_progress)
    ...
    with tab_twin:
        ...
        if twin_rows:
            ...
            st.dataframe(twin_rows, **streamlit_width_arg())
        ...
        if recommender_rows:
            st.dataframe(recommender_rows, **streamlit_width_arg())
        else:
        ...
        st.dataframe(filtered, **streamlit_width_arg())
