...
APP_SUBTITLE = (
    ...
    "digital emotional twin logging, adaptive recommendation, and free Hugging Face integration."
)
...
FALLBACK_ACTIONS = {
    "happy": "Save the moment by writing one win, one gratitude, and one next goal.",
    ...
    "energetic": "Channel the energy into one meaningful task or a short movement break.",
}
...
TONE_GUIDES = {
    "Therapist": (
        "Be warm, grounded, reflective, and structured. Ask meaningful questions, "
        "avoid cliches, and encourage one small actionable step."
    ),
    "Friendly": (
        "Be caring, conversational, and positive. Sound like a smart supportive friend, "
        "not a generic FAQ bot."
    ),
    "Motivational": (
        "Be energising, disciplined, and forward-looking. Push the user gently toward action "
...
DATASET_GUIDE = [
    {
        ...
        "note": "Keep sample size small on 8 GB RAM laptops.",
    },
    {
        "dataset": "EmoNet-Face-Big",
        "year": "2025",
        "type": "Synthetic face emotion",
        "best_use": "Experiments or pretraining-style sampling",
    ...
    },
    {
        ...
        "best_use": "Project documentation and future scope",
    ...
    },
    {
        ...
        "year": "2023",
        "type": "Multimodal research",
        "best_use": "Literature comparison and future scope",
        "note": "Use only if you already have access.",
    },
]
...
def update_last_feedback(user: str, item_id: str, feedback: str) -> None:
    ...
    fieldnames = list(rows[0].keys())
    write_csv_rows(TWIN_LOG_PATH, fieldnames, rows)
...
def get_user_twin_summary(user: str, limit: int = 12) -> str:
    ...
    return (
        ...
        f"Recent mood sequence: {recent_sequence or 'none'}. "
...
def ethical_monitor_report(
    ...
    if face_result.get("method") == "opencv_heuristic":
        warnings.append("Facial analysis is using heuristic fallback. Train the .pth model for stronger accuracy.")
    ...
    if dominant_score < 0.25:
        warnings.append("Signals are mixed, so response confidence is moderate.")
    warnings.append("This system offers supportive guidance, not a clinical diagnosis.")
...
def build_chat_system_prompt(
    ...
    return textwrap.dedent(
        ...
        Requirements:
        1. Sound warm, intelligent, and specific to the user's situation.
        2. Avoid generic FAQ style answers.
        3. Give one short supportive reflection.
        4. Ask exactly three important reflective questions.
        5. Suggest one practical next action for the next 10-20 minutes.
...
def local_transformers_chat(system_prompt: str, user_prompt: str) -> str | None:
    ...
    prompt = (
        ...
        "Write a concise response with one reflection, three questions, and one next action."
    )
    try:
...
def render_setup_tab() -> None:
    ...
    st.markdown(
        textwrap.dedent(
            ...
            2. Create these folders only when you need them:
               - `dataset/`
               - `models/`

            3. Open the integrated terminal in VS Code and create a virtual environment.
               Windows PowerShell:
               ```powershell
               python -m venv .venv
            ...
            5. Optional free Hugging Face API setup:
            ...
            7. Open the localhost URL printed in the terminal, usually:
               - `http://localhost:8501`
            """
        )
    )
    st.markdown("### What goes inside `requirements.txt`")
    st.code(
        "\n".join(
            [
                "torch",
    ...
    st.markdown("### Why Hugging Face is the recommended free API option")
    st.markdown(
        textwrap.dedent(
            f"""
            - It supports **free account creation** with a **free read token**.
            - It gives an easy path for **free inference API use** or **local transformers fallback**.
            - It avoids paid OpenAI usage for this project.
            - It fits this single-file PyTorch major project better than paid GPT-only workflows.
            - Current active provider mode in this app: **{get_chat_provider_name()}**
            """
        )
    )
    st.markdown("### Spotify note")
    st.info(
        "To keep the project fully free and lightweight, this app uses public Spotify search links "
        "instead of the paid or rate-limited developer API flow."
    )
    ...
    st.markdown(
        textwrap.dedent(
            ...
            - Host the app locally or on a free cloud runtime.
            - Open the Streamlit URL in Chrome or Edge on Android.
            - Use **Add to Home Screen** for a near-app experience.
            - If you later need a real APK, wrap the same URL with a free WebView tool or Android Studio WebView project.
            """
        )
    )
    ...
    st.markdown(
        textwrap.dedent(
            """
            - Recent CNN upgrade: EfficientNetV2-S with global average pooling and PyTorch Grad-CAM
            - Multimodal fusion: face + voice + text + emoji
            - Digital Emotional Twin: persistent user-state logging
            - Reinforcement-learning-style recommendation without repetition
            - Ethical AI layer: confidence transparency and non-diagnostic guidance
            - Future scope: avatar lip sync, stronger speech emotion models, multilingual support, mobile packaging
            """
        )
    )
    st.markdown("### Suggested public dataset references for your report")
...
def run_cli() -> int:
    ...
    parser.add_argument(
        ...
        help="Custom destination for the exported catalog CSV.",
    )
...
def main() -> None:
    ...
    with tab_live:
        ...
        with left:
            ...
            if hasattr(st, "audio_input"):
                ...
                st.info("Your Streamlit version does not support `st.audio_input`. Update Streamlit for browser mic capture.")
    ...
    with tab_train:
        ...
        st.caption(
            "This training block expects a local folder dataset/train, dataset/val, and dataset/test. "
            "It saves the trained model as a single .pth checkpoint."
        )
        ...
        if summary_rows:
            ...
            st.info("No local dataset found yet. Create the dataset folders described in the setup tab.")
    ...
    with tab_twin:
        ...
        if recommender_rows:
            st.dataframe(recommender_rows, use_container_width=True)
        else:
